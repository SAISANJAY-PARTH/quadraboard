...
if is_range_market:
    ...
    if raw_signal == "STRONG BUY":
        ...
        signal_override = f"⚠️ ADX={adx_v:.1f} < 20 — downgraded STRONG BUY → BUY. Use smaller size."
    elif raw_signal == "STRONG SELL":
        final_signal = "SELL (Weak Trend)"
        signal_override = f"⚠️ ADX={adx_v:.1f} < 20 — downgraded STRONG SELL → SELL. Use smaller size."
    else:
...
try:
    ...
    if len(df_ml) > 30:
        ...
        if "Stable" in current_regime:
            if is_range_market:
                st.warning("📦 Stable regime but **range-bound** — best strategy: **mean reversion / range trading**, NOT trend following.")
            else:
                st.success("Stable market — Trend-following strategies favoured (if ADX > 20).")
        elif "Moderate" in current_regime:
            st.warning("Moderate volatility — Use tighter stops. Range or weak-trend conditions.")
        else:
            st.error("High volatility — Reduce position size, widen stops or stay out.")

        # Regime consistency note
        if "Stable" in current_regime and avg_ret < -0.002:
            st.info(f"ℹ️ Regime is 'Stable' but avg return = {avg_ret*100:.3f}% (negative). This means: **stable downtrend** or slow drift — not necessarily safe.")

except Exception as e:
    st.warning(f"ML Regime detection failed: {e}")

# ===========================================================
# 🌳 DECISION TREE CLASSIFIER  (FIXED: accuracy guard added)
# ===========================================================
...
| **7–10** | Very deep | Overfit ⚠️ | Usually bad |

> ⚠️ **Warning:** Confidence = 100% but Accuracy ≈ 50% = overfitting.
> ✅ **Healthy:** Confidence 55–72%, Accuracy > 52%.
---
""")
        ...
        if dt_depth <= 3:
            st.info(f"📐 Depth {dt_depth} — Simple tree. Lower chance of overfitting.")
        elif dt_depth <= 5:
            st.success(f"📐 Depth {dt_depth} — Balanced tree. Recommended.")
        elif dt_depth <= 7:
            st.warning(f"📐 Depth {dt_depth} — Complex tree. Watch for overfitting.")
        else:
            st.error(f"📐 Depth {dt_depth} — Very deep tree. High overfitting risk.")
        ...
        # ── FIX: Proper accuracy health check ──
        if dt_acc < 0.50:
            st.error(f"""
🚨 **Model Accuracy = {dt_acc*100:.1f}% (Below Random Chance)**

This model is performing **worse than a coin flip**. This means:
- It has NOT learned a real pattern
- Prediction output is **unreliable / do NOT use**
- This often happens in range-bound, choppy markets
- The current market (ADX={adx_v:.1f}) may have little predictable structure

**What to do:** Change time period, reduce Max Depth to 2-3, or accept that the market is currently unpredictable.
""")
        elif dt_acc < 0.53:
            st.warning(f"⚠️ Accuracy {dt_acc*100:.1f}% is only marginally above random (50%). Treat prediction with low confidence.")
        elif dt_confidence >= 95:
            st.error(f"🚨 Confidence {dt_confidence:.0f}% is suspiciously high with accuracy {dt_acc*100:.1f}% — likely **overfitting**. Lower depth.")
        elif dt_confidence >= 80:
            st.warning(f"⚠️ Confidence {dt_confidence:.0f}% is high — possible overfitting. Test Accuracy = {dt_acc*100:.1f}%.")
        elif dt_confidence >= 55 and dt_acc >= 0.53:
            st.success(f"✅ Confidence {dt_confidence:.0f}% + Accuracy {dt_acc*100:.1f}% — model looks healthy.")
        else:
            st.info(f"ℹ️ Confidence {dt_confidence:.0f}% is low — market may be in a choppy/unpredictable phase.")

        # ── Prediction display with accuracy caveat ──
        if dt_acc < 0.50:
            st.error(f"🌳 Decision Tree says: **{'UP' if dt_pred_label==1 else 'DOWN'}** — ⚠️ BUT accuracy={dt_acc*100:.1f}%, prediction is NOT reliable")
        elif dt_pred_label == 1:
            st.success(f"🌳 Decision Tree says: **UP** with {dt_confidence:.1f}% confidence (accuracy: {dt_acc*100:.1f}%)")
        else:
            st.error(f"🌳 Decision Tree says: **DOWN** with {dt_confidence:.1f}% confidence (accuracy: {dt_acc*100:.1f}%)")
        ...
        if len(y_test) >= roll_window:
            ...
            fig_roll.add_hline(y=50, line_dash="dash", line_color="red",
                               annotation_text="50% = Random (below = worse than random)")
            fig_roll.update_layout(
                template="plotly_dark", height=250,
                title=f"Rolling {roll_window}-Bar Accuracy (%) — Red line = random baseline",
...
# ===========================================================
# 📈 LOGISTIC REGRESSION  (FIXED: accuracy guard added)
# ===========================================================
...
try:
    ...
    if len(df_lr) < 60:
        ...
        lr_up_prob       = lr_pred_proba[1] * 100
        ...
        # ── FIX: Proper accuracy health check for LR ──
        if lr_acc < 0.50:
            st.error(f"""
🚨 **LR Accuracy = {lr_acc*100:.1f}% (Below Random Chance)**

Logistic Regression is performing worse than a coin flip. The prediction output is **unreliable**.
This is common in range-bound markets with low ADX (currently ADX={adx_v:.1f}).
Do NOT use this prediction for trading decisions.
""")
        elif lr_acc < 0.53:
            st.warning(f"⚠️ LR Accuracy {lr_acc*100:.1f}% — barely above random. Treat with caution.")
        elif abs(lr_up_prob - 50) < 5:
            st.warning(f"⚠️ P(UP)={lr_up_prob:.1f}% is very close to 50% — model is uncertain, essentially a coin flip.")
        else:
            if lr_pred_label == 1:
                st.success(f"📈 Logistic Regression says: **UP** — P(Up)={lr_up_prob:.1f}% (accuracy: {lr_acc*100:.1f}%)")
            else:
                st.error(f"📉 Logistic Regression says: **DOWN** — P(Down)={100-lr_up_prob:.1f}% (accuracy: {lr_acc*100:.1f}%)")
        ...
        # ── COMBINED ML SUMMARY (FIXED: includes accuracy reliability check) ──
        ...
        dt_reliable = dt_acc >= 0.53
        lr_reliable = lr_acc >= 0.53
        reliable_votes_up   = sum([1 for v, r in [(dt_vote, dt_reliable), (lr_vote, lr_reliable)] if v == "UP" and r])
        reliable_votes_down = sum([1 for v, r in [(dt_vote, dt_reliable), (lr_vote, lr_reliable)] if v == "DOWN" and r])
        n_reliable = sum([dt_reliable, lr_reliable])

        col_m1, col_m2, col_m3, col_m4 = st.columns(4)
        col_m1.metric(
            "Decision Tree",
            f"{'📈' if dt_vote=='UP' else '📉'} {dt_vote}",
            f"Acc:{dt_acc*100:.1f}% {'✅' if dt_reliable else '❌ Unreliable'}"
        )
        col_m2.metric(
            "Logistic Reg",
            f"{'📈' if lr_vote=='UP' else '📉'} {lr_vote}",
            f"Acc:{lr_acc*100:.1f}% {'✅' if lr_reliable else '❌ Unreliable'}"
        )
        col_m3.metric(
            "ML Consensus",
            f"{'🟢 UP' if votes_up >= 2 else ('🔴 DOWN' if votes_up == 0 else '🟡 SPLIT')}"
        )
        col_m4.metric(
            "Reliable Models",
            f"{n_reliable}/2",
            "Models above 53% accuracy"
        )

        if n_reliable == 0:
            st.error("""
❌ **Neither ML model is reliable** (both accuracy < 53%)

Both models are performing at or below random chance. This is a direct signal that:
- The market has **no learnable short-term pattern** right now
- Conditions are choppy/range-bound
- **Do NOT use ML predictions for trading in current conditions**

This is NOT a system failure — it's the system correctly telling you the market is unpredictable.
""")
        elif n_reliable == 1:
            st.warning(f"⚠️ Only 1 of 2 ML models is reliable. Use with caution. {'DT' if dt_reliable else 'LR'} has acceptable accuracy.")
        else:
            if votes_up == 2:
                st.success("✅ Both reliable ML models agree: **BULLISH** next day")
            elif votes_up == 0:
                st.error("❌ Both reliable ML models agree: **BEARISH** next day")
            else:
                st.warning("⚠️ Reliable ML models disagree — Mixed signal, exercise caution")

        # ── FINAL COMBINED VERDICT ──
        st.markdown("---")
        st.subheader("🏆 Final Professional Verdict")

        # Combine: Technical + ML + Regime
        tech_bias   = "BULLISH" if bull_score > bear_score else ("BEARISH" if bear_score > bull_score else "NEUTRAL")
        ml_bias     = ("BULLISH" if reliable_votes_up > reliable_votes_down
                       else ("BEARISH" if reliable_votes_down > reliable_votes_up else "UNCERTAIN"))
        regime_bias = "STABLE" if "Stable" in current_regime else ("VOLATILE" if "Volatile" in current_regime else "MODERATE")

        final_verdict_lines = []
        final_action        = ""

        if is_range_market:
            final_action = "📦 RANGE TRADE — Do not trend-follow"
            final_verdict_lines = [
                f"• Market is **range-bound** (ADX={adx_v:.1f}, BB squeeze, EMA compression)",
                f"• Technical score = {tech_bias} but **unreliable** in range conditions",
                f"• ML models: {'unreliable (< 53% accuracy)' if n_reliable == 0 else f'{n_reliable}/2 reliable, consensus = {ml_bias}'}",
                f"• Best strategy: **fade extremes** — buy near {symbol}{last['Support']:.2f}, sell near {symbol}{last['Resistance']:.2f}",
                f"• Wait for ADX > 20 + volume expansion for a real breakout trade",
            ]
        elif is_weak_trend:
            if tech_bias == ml_bias and ml_bias != "UNCERTAIN":
                final_action = f"⚠️ CAUTIOUS {tech_bias} — Weak trend, reduce size by 50%"
                final_verdict_lines = [
                    f"• Weak trend detected (ADX={adx_v:.1f})",
                    f"• Technical and ML both agree: **{tech_bias}**",
                    "• Reduce position size by 50%, take profits earlier",
                ]
            else:
                final_action = "⚠️ WAIT — Weak trend + conflicting signals"
                final_verdict_lines = [
                    f"• Weak trend (ADX={adx_v:.1f}) + Technical={tech_bias}, ML={ml_bias}",
                    "• No high-confidence setup. Best to **wait for clarity**.",
                ]
        else:
            # Strong trend: trust tech signal
            if tech_bias == "BULLISH":
                final_action = "🚀 TRENDING BUY — Full size trade" if adx_strong else "🟢 BUY — Moderate size"
            elif tech_bias == "BEARISH":
                final_action = "🔻 TRENDING SHORT — Full size trade" if adx_strong else "🔴 SELL — Moderate size"
            else:
                final_action = "⚠️ WAIT"
            final_verdict_lines = [
                f"• Strong trend confirmed (ADX={adx_v:.1f})",
                f"• Technical = {tech_bias}, ML = {ml_bias}, Regime = {regime_bias}",
                f"• R:R = 1:{rr} using ATR-based levels above",
            ]

        if "BUY" in final_action or "BULLISH" in final_action:
            st.success(f"## {final_action}")
        elif "SELL" in final_action or "SHORT" in final_action or "BEARISH" in final_action:
            st.error(f"## {final_action}")
        else:
            st.warning(f"## {final_action}")

        for line in final_verdict_lines:
            st.markdown(line)

        st.info("""
ℹ️ **How the Final Verdict is determined:**
- Technical Engine score (12 indicators across trend/momentum/volume)
- ADX gate: overrides directional signals in range/weak-trend conditions  
- ML reliability check: only counts models with accuracy > 53%
- Market regime from clustering: context-adjusts strategy recommendation
- Range detection: requires 2 of 3 signals (ADX < 15, BB squeeze, EMA compression)
        """)

except Exception as e:
    st.warning(f"Logistic Regression / Final Verdict failed: {e}")
